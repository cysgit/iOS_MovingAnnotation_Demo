 ...
 * @param URLTemplate URLTemplate是一个包含"{x}","{y}","{z}","{scale}"的字符串,"{x}","{y}","{z}","{scale}"会被tile path的值所替换，并生成用来加载tile图片数据的URL 。例如：@link http://server/path?x={x}&y={y}&z={z}&scale={scale}
...
///此category中的方法来自定义加载MKTileOverlay
